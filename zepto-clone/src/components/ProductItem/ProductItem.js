...
const ProductItem = (props) => {
  ...
  if (product === undefined) {
    ...
    console.log(product,data,"product id, data")
    ...
    <div className="ordered-product-card">
      <div className="cp-card-container">
        <div className="d-flex">
          ...
          />
          <div>
            <p className="cp-product-name">{product.productData.productName}</p>
    ...
    <Link to={`/sub-category/${data.id}/1`}>
      <div>
        <img
          alt="product"
          src={data.image}
          className={data.id === "1" ? classes["listItemFirst"] : classes["listItem"]}
        />
      </div>
    </Link>
  );
